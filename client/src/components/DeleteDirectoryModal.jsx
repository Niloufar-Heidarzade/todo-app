  ...
  return (
    <>
      ...
      <div className="fixed top-1/2 left-1/2 z-60 transform -translate-x-1/2 -translate-y-1/2  bg-gray-200 w-70 sm:w-98 h-41 rounded-sm py-5 px-4" ref={modalRef}>
        ...
        <p className="text-sm text-gray-500 mt-5">This directory will be deleted permanently.</p>
        <button className="text-sm text-gray-500 cursor-pointer mr-1 w-20 h-9 rounded-sm hover:border-2 hover:border-violet-500" onClick={() => dispatch(closeDeleteDirectoryModal())}>
          Cancel
        </button>
        <button className="bg-violet-500 text-white text-sm w-20 h-9 rounded-sm mt-5 cursor-pointer transform hover:bg-violet-700 duration-200 " onClick={() => {
          taskList.forEach((task) => task.directory === current && dispatch(deleteTask(task.id)))
