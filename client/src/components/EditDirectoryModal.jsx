...
function EditDirectoryModal() {
  ...
  return (
    <>
      ...
      <form className="z-60 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-200 w-70 sm:w-90 h-46 rounded-sm py-5 px-4" ref={modalRef} onSubmit={handleSubmit(onSubmit)}>
        ...
        <label htmlFor="directory" className="block mt-3 text-sm text-gray-600">Title</label>
        <input type="text" placeholder={current.charAt(0).toUpperCase() + current.slice(1)} className="bg-gray-50 w-full h-9 rounded-sm pl-2 text-sm focus:outline-none focus:border-1 focus:border-violet-500" id="directory" {...register("directoryNewName" , {
